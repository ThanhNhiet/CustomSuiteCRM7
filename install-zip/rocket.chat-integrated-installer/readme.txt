...
**Important Warning 1**: If you select the template overwrite option, please backup your existing theme files before installation:
```
themes/SuiteP/tpls/_headerModuleList.tpl
themes/SuiteP/tpls/footer.tpl
```

**Important Warning 2**: If you already have custom API at custom/application/Ext/Api/V8, please backup your directory before installation. Here is files will add into that directory.
```
application
└───Ext
    ├───Api
...
- ${rc_oauth_client_id} with OAuth client ID
- ${rc_oauth_client_secret} with OAuth client secret
